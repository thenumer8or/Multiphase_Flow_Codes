      ...
      if(myid == 0)then
       do ip = 1,npart
         if(ovlpflagt(ip) > 0)then
           ypglb(1,ip) = real(nx)*ranpp(iseedp,iyp,ivp)
            ...
            iflag = 1
             if(istep .gt. irelease)then
              write(*,*)'in ovlpchk hgap<0',hgap,' istep ',istep,' ip,id,ii ',ip,id,ii, &
                        'myid,nps,ipglp(ip),ipglb(ii)',myid,nps,ipglb(ip),ipglb(ii) 
             endif
            exit
          end if
        end do
...
! if gravity is desired in "-y" dir
        forcep(2,ip) = forcep(2,ip) - volp*rhog
...
!===========================================================================
      ...
      integer npsflag, npsflagt, npst, id, ovlpcnt
...
! periodic box
      do ip = 1,nps
        ...
        if(xc >= real(nx)) yp(1,ip) = yp(1,ip) - real(nx)
        if(xc < 0.0) yp(1,ip) = yp(1,ip) + real(nx)
